...
- IPC 우선: 로컬 전용 환경에서 IPC(JSONL)를 기본 통신 방식으로 채택한다.
- DB 우선: 대량 파일 처리와 반복 검색을 위해 초기부터 인덱스(DB)를 사용한다.
...
- 내부 타입명 `Preset`은 현재 템플릿(전체)을 의미하며, 프리셋(변수 1개)과 구분되는 점을 문서로 명시한다. 추후 `Template`/`Preset` 타입 분리 리팩터링을 고려한다.
...
- tags(image_id, tag)
- matches(image_id, variable, status, values_json)

### 태그 저장 방식
- 기본: tags 테이블에 정규화된 태그를 행 단위 저장
- 필요 시 images.tags_json 병행 저장(디버깅/덤프용)

### 썸네일 캐시
- 캐시 폴더: cache/thumbs/
- 키: 경로 기반 해시(중복 방지)
- 만료 정책: 용량 상한 + LRU(초기엔 단순 전체 삭제 가능)

### IPC 메시지 정책(요약)
- 작업별 id로 진행/결과를 구분
- 취소 요청은 청크 단위로 안전 종료
- 오류는 error 메시지로 전달하고 작업 상태를 종료 처리
