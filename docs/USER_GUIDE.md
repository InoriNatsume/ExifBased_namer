...
### 방법 1: exe 파일 (권장)
1. `nai-classifier-server.exe` 실행
...
## 폴더 분류
- 단일 변수 또는 계층별 변수 기준으로 폴더를 생성하고 이동합니다.
- 기본 템플릿은 `[value]` 입니다.
- 재개 모드는 `.nai_resume_move.txt`를 사용합니다.

### 계층별 분류
- `variable_tree` 배열로 여러 변수를 순서대로 지정하면 계층 폴더가 생성됩니다.
- 예: `["character", "emotion"]` → `캐릭터명/감정명/` 폴더 구조

### 부분 분류 (PARTIAL)
- 1층 변수만 매칭되고 2층은 실패한 경우, 1층 폴더에만 분류됩니다.
- 결과 상태가 PARTIAL(부분 분류)로 표시됩니다.
...
|------|------|------|
| OK | 모든 변수 매칭 성공 | 초록 |
| PARTIAL | 일부 변수만 매칭 (계층별 분류 시) | 청록 |
| UNKNOWN | 매칭되는 값 없음 | 노랑 |
| CONFLICT | 2개 이상 값과 매칭 | 빨강 |
| ERROR | 처리 중 오류 발생 | 빨강 |
| SKIP | 건너뜀 (재개 모드) | 회색 |
